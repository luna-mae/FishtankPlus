...
function loadArrive(callback) {
  ...
  script.src = chrome.runtime.getURL('libs/arrive.js');
  script.onload = callback;
  document.head.appendChild(script);
}

function loadKeycode(callback) {
  const script = document.createElement('script');
  script.src = chrome.runtime.getURL('libs/keycode.js');
  script.onload = callback;
  document.head.appendChild(script);
}
...
function initialize() {
  ...
  function checkLoadingStatus() {
    if (librariesLoaded && typeof window.arrive === 'function') {
      setupCollapseButton();
